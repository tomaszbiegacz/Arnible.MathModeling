...
namespace Arnible.MathModeling.Test.Geometry
{
  public class INumberRangeDomainExtensionsTests
  {
    ...
    public void EmptyRatio()
    {
      ...
      Assert.Equal(default, _domain.GetValidTranslation(current, _directions.First(d => d.Angles.Single() == 2), 0));
    }
    ...
    {
      NumberVector current = new NumberVector(0, 0);
      Assert.Equal(new NumberTranslationVector(0, 1), _domain.GetValidTranslation(current, _directions.First(d => d.Angles.Single() == 2), 0.5));
    }
    ...
    public void PositivePartial()
    {
      ...
      Assert.Equal(new NumberTranslationVector(0, 0.5), _domain.GetValidTranslation(current, _directions.First(d => d.Angles.Single() == 2), 0.5));
    }
    ...
    public void NegativePartial()
    {
      ...
      Assert.Equal(new NumberTranslationVector(-0.5, -0.5), _domain.GetValidTranslation(current, _directions.First(d => d.Angles.Single() == 1), 0.5));
    }
  }
}
